...
MODULE_DESCRIPTION("Device driver for various TV and TV+FM radio tuners (TL880 Modification)");
MODULE_AUTHOR("Ralph Metzler, Gerd Knorr, Gunther Mayer (TL880 Modification by Mike Bourgeous)");
MODULE_LICENSE("GPL");
...
static struct tunertype tuners[] = {
	...
	{ "Temic PAL* auto + FM (4009 FN5)", TEMIC, PAL,
	  16*141.00, 16*464.00, 0xa0,0x90,0x30,0x8e,623},
  	{ "Philips 1236D ATSC/NTSC dual in",Philips,ATSC,
			...
			unsigned short flags, int kind)
{
	...
	if (type >= 0 && type < TUNERS) {
		...
		if(type == TUNER_PHILIPS_ATSC)
			t->mode = 2; // default NTSC RF input 1
	} else {
		t->type = -1;
	}
...
static int tuner_probe(struct i2c_adapter *adap)
{
	...
	this_adap = 0;
	if (adap->id == (I2C_ALGO_BIT | I2C_HW_B_TL880)) /* Changed from BT848 by Mike Bourgeous */
...
tuner_command(struct i2c_client *client, unsigned int cmd, void *arg)
{
	...
	case VIDIOCSCHAN:
	{
		...
		if (t->type == TUNER_PHILIPS_ATSC) {
			t->mode  = (vc->norm == VIDEO_MODE_ATSC) ? 0 : 2;
